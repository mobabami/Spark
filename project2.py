...
#Starting from Customer to print all nested records related to each customer
def printCustomer(customer):
    print("Customer: "+ customer[0])
    tempCust = sc.parallelize(customer)
    joinCustomerOrder = tempCust.join(orders.map(lambda l: (l["CustomerId"],l)))
    joinCustomerOrder.foreach(lambda l: printOrder(l))

customersKeyValuePair.foreach(lambda l: printCustomer(l))
#for l in customersKeyValuePair.collect():
#    printCustomer(l)